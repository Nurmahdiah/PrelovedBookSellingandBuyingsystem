...
<div class="container">
<h4><a href="homePage.jsp">Buyer</a> / <a href="buyBook.jsp"> Preloved Book Details</a> / Add to Cart </h4>
<br>
...
<div class="card h-100">
	<div class="card-body">
		...
		<h5 class="user-name"><b>Title : </b> We Begin at the End</h5>
		<br>
...
<!-- Nanty untuk submit book to cart ke database, akan buat form, ambik data dari buku dari database table preloved book guna idbook, 
then type input jadi "hide" supaya yang input ask kepada user just quantity buku sahaja.
 nanty masuk dalam table order dan order details which order status tu default kepada unpaid-->
<br><br>
...
<div class="card h-100">
	...
	<div class="card-body">
	<form action="buyerCartAdd" method="post" onsubmit="cart()">
	
	<h6 class="mb-2 text-primary">Add to Cart</h6>
	...
	<input type="hidden" name="amount" id="amount" value="<%=resultset.getString("price")%>">
	...
	%>
	<input type="hidden" name="datecart" id="datepicker" value="<%=strDate%>">
	
	<input type="hidden" name="defaultstatus" id="defaultstatus" value="cartunpaid">
	
	<input type="hidden" name="buyerid" id="buyerid" value="<%=session.getAttribute("buyerLogin")%>">
	
	<input type="hidden" name="sellerid" id="sellerid" value="<%=resultset.getString("sellerEmail")%>">
...
function cart(){
	...
	if (quantity > stock){
		alert("Book stock is not enough");
		return false;
	}
	else
		return true;
